...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((name, phone) in mapB) {
        if (!mapC.contains(name)) mapC[name] = phone
    }
    for ((name, phone) in mapC) {
        if (name in mapB && mapC[name] != mapB[name]) mapC[name] = (phone + ", " + mapB[name])
    }
    return mapC
}
...
fun propagateHandshakes(friends: Map<String, Set<String>>): Map<String, Set<String>> {
    ...
    for ((name, set) in map) {
        for (element in set) {
            if (element in map)  {s = (friends[element]!!.union(set)); map[name] = s - name}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in 0..(list.size - 1)) {
        ...
        if (i > list.indexOf(number - list[i])) a = Pair(list.indexOf(number - list[i]), i)
        else a = Pair(i, list.indexOf(number - list[i]))
        }
    }
    return a
}
